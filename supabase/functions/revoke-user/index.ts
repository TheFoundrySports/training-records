...
Deno.serve(async (req) => {
  ...
  let action: string
  try {
    const body = (await req.json()) as { user_id?: string; action?: string }
    userId = body.user_id ?? ''
    action = body.action ?? ''
  } catch {
  ...
  if (action !== 'ban' && action !== 'unban') {
    return errorResponse('BAD_REQUEST', 'action must be "ban" or "unban"', 400)
  }
  ...
  const { error: updateError } = await supabaseAdmin.auth.admin.updateUserById(userId, {
    ...
    data: action === 'ban'
      ? { ...targetUser.user?.user_metadata, banned: true, banned_at: new Date().toISOString() }
      : { ...targetUser.user?.user_metadata, banned: false, banned_at: null },
  })
