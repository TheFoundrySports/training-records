 ...
 */
pages_fixture_1.test.describe('Workout with WOD format', () => {
    (0, pages_fixture_1.test)('selects "For Time" format and movement section appears', async ({ workoutFormPage, page, }) => {
        ...
        await (0, pages_fixture_1.expect)(page.getByTestId('wod-form-section-for-time-payload')).toBeVisible();
        // "Add movement" button should be visible
        await (0, pages_fixture_1.expect)(page.getByRole('button', { name: /add movement/i })).toBeVisible();
        ...
        await (0, pages_fixture_1.expect)(row0.getByTestId('exercise-picker-input')).toBeVisible();
        // Fill rep scheme
        await row0.getByLabel(/rep scheme/i).fill('21-15-9');
        await (0, pages_fixture_1.expect)(row0.getByLabel(/rep scheme/i)).toHaveValue('21-15-9');
    });
    (0, pages_fixture_1.test)('creates a for-time workout with one movement and saves', async ({ workoutFormPage, page, }) => {
        await workoutFormPage.gotoNew();
        ...
        await (0, pages_fixture_1.expect)(page.locator('#exercise-options option[value="Deadlift"]')).toBeAttached({
            timeout: 10_000,
        });
        ...
        await (0, pages_fixture_1.expect)(row0.locator('[data-testid="exercise-id-0"]')).toHaveValue(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/, { timeout: 5_000 });
        // Rep scheme
        await row0.getByLabel(/rep scheme/i).fill('21-15-9');
        // Save — exerciseId should be resolved; expect navigation to /workouts
        await workoutFormPage.saveAndExpectSuccess();
    });
    (0, pages_fixture_1.test)('shows validation error if movements list is empty when format is set', async ({ workoutFormPage, page, }) => {
        await workoutFormPage.gotoNew();
        ...
        await (0, pages_fixture_1.expect)(page).toHaveURL('/workouts/new');
    });
});
