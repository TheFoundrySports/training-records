 ...
 */
const FIT_FIXTURE = path_1.default.join(__dirname, 'fixtures', 'activity.fit');
...
pages_fixture_1.test.describe('Garmin import: full upload flow', () => {
    (0, pages_fixture_1.test)('upload .fit file → metrics panel appears → evaluation card loads', async ({ page, workoutsPage, workoutFormPage, garminImportPage, }) => {
        ...
        await (0, pages_fixture_1.expect)(page.getByRole('dialog')).not.toBeVisible({ timeout: 10_000 });
        // ── Step 5: Assert TrainingMetricsPanel became visible ───────────────────
        await garminImportPage.expectMetricsPanelVisible();
        await (0, pages_fixture_1.expect)(page.getByText('Garmin Metrics')).toBeVisible();
        // ── Step 6: Assert AIEvaluationCard shows content ────────────────────────
        await garminImportPage.expectEvaluationCardVisible();
        await (0, pages_fixture_1.expect)(page.getByText('AI Training Evaluation')).toBeVisible();
    });
});
pages_fixture_1.test.describe('Garmin import: re-import flow', () => {
    (0, pages_fixture_1.test)('re-import shows replacement warning before proceeding', async ({ page, workoutsPage, workoutFormPage, garminImportPage, }) => {
