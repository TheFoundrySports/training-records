...
- **NEVER commit directly to `main`** — this is a hard rule with zero exceptions
- All commits must go through a `feat/` or `fix/` branch and merged via PR
- Before committing, always run `git branch --show-current` to verify you're on the correct branch
- If accidentally on `main`, create a new branch: `git checkout -b fix/my-fix` and amend/rebase the commits there
